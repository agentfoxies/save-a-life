...
const SupportChat = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      newSocket.emit('join_room', { roomId, senderType: 'support', displayName: supportName })
    })
    ...
    newSocket.on('user_typing', (data: { displayName: string }) => {
    ...
    return () => {
      newSocket.emit('stop_typing', { roomId })
      newSocket.close()
    }
  }, [roomId])
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-screen flex flex-col pt-16">
      <div className="glass-card border-b border-gray-200 dark:border-gray-700 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <button onClick={() => navigate('/dashboard')} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
              <HiArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h2 className="font-semibold">Support Chat</h2>
              <p className="text-xs text-gray-500">Room: {roomId?.substring(0, 8)}... • Support Mode</p>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <HiExclamationTriangle className="w-16 h-16 text-gray-300 mx-auto mb-4" />
        ...
        {isTyping && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center space-x-2 text-sm text-gray-500">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-3">
        <form onSubmit={handleSendMessage} className="flex items-end space-x-2">
          <textarea value={newMessage} onChange={(e) => { setNewMessage(e.target.value); handleTyping() }}
            ...
            className="flex-1 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
            style={{ maxHeight: '120px' }} />
          <motion.button type="submit" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            disabled={!newMessage.trim()}
            className="p-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-full text-white">
