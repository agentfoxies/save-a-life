...
const ChatRoom = () => {
  ...
  useEffect(() => {
    ...
    socket.on('message_deleted', (data: { messageId: string }) => {
      // Remove deleted message from state handled by context
    })
      ...
      <div className="glass-card border-b border-gray-200 dark:border-gray-700 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <button onClick={handleLeave} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
              <HiArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-white">Support Chat</h2>
              <p className="text-xs text-gray-500">{displayName} • Private Room</p>
            </div>
          </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-3">
        <form onSubmit={handleSendMessage} className="flex items-end space-x-2">
          ...
          <textarea value={newMessage} onChange={(e) => { setNewMessage(e.target.value); handleTyping() }}
            ...
            className="flex-1 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-primary-500"
            style={{ maxHeight: '120px' }} />
          <motion.button type="submit" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            disabled={!newMessage.trim()}
            className="p-3 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 rounded-full text-white">
