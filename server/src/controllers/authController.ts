...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!admin.approved) return res.status(403).json({ error: 'Your account is pending approval from the owner.' });
...
export const approveAdmin = async (req: any, res: Response) => {
  try {
    if (req.admin.role !== 'owner') return res.status(403).json({ error: 'Only owner can approve admins' });
...
export const removeAdmin = async (req: any, res: Response) => {
  try {
    ...
    res.json({ message: 'Admin removed. All their sessions are now invalid.' });
  } catch (error) {
...
export const authMiddleware = async (req: any, res: Response, next: any) => {
  try {
    ...
    if (!admin || !admin.approved) {
      return res.status(403).json({ error: 'Account not approved or deleted' });
    }
